...
def calBMI(event):
    ...
    if resultCal >= 30:
        print("อ้วนมากกกก %.2f" % (resultCal))
        labelResult.configure(text=("อ้วนมากกกก BMI = %.2f " % (resultCal)),font = ("Angsana,10"),fg = "red")
    elif resultCal < 30 and resultCal >=25:
        print("อ้วน", resultCal)
        labelResult.configure(text=("อ้วน BMI = %.2f " % (resultCal)), font=("Angsana,10"),fg = "red")
    elif resultCal < 25 and resultCal >= 23:
        print("น้ำหนักเกิน", resultCal)
        labelResult.configure(text=("น้ำหนักเกิน BMI = %.2f "% (resultCal)), font=("Angsana,10"),fg = "red")
    elif resultCal < 23 and resultCal >= 18.6:
        print("น้ำหนักเหมาะสม", resultCal)
        labelResult.configure(text=("น้ำหนักเหมาะสม BMI = %.2f "% (resultCal)), font=("Angsana,10"),fg = "red")
    else:
        print("ผอมเกิน", resultCal)
        labelResult.configure(text=("ผอมเกิน BMI = %.2f "% (resultCal)), font=("Angsana,10"),fg = "red")
